...
fn App() -> Html {
    ...
    let usersname_handle = use_state(String::default);
    let username = (*usersname_handle).clone();
        ...
        if let Some(ws_msg) = &**ws_message {
            ...
            match websocket_message.message_type {
                ...
                WebSocketMessageType::UsernameChange => {
                    let username = websocket_message.username.expect("Missing username");
                    usersname_handle.set(username);
                }
            }
        }
    });
